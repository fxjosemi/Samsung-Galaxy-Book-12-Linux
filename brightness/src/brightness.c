 ...
 *      checked AUX initialization and byte-safe interfaces.
 *
...
int accessParaAuxRegs(aux_t aux)
{
    ...
    for (int i=0; i < 5; i++) {
        ...
        for (int i=0; cmd[i]; i++)
            if (dpAuxWrite(aux, 0x490, cmd[i]) < 0)
                return 1;
...
unsigned char __readDisplayRegister(aux_t aux, unsigned char reg, int type)
{
...
unsigned char __readDisplayRegister2(aux_t aux, unsigned char val[3])
{
...
{
    const unsigned char indexes[3] = {0x67, 0x69, 0x6B};
    ...
    for (int i=0; i < 3; i++) {
        unsigned char outA, outB, idx=indexes[i];
 ...
 */
unsigned char __brightnessRegisters[][3] = {
    {2, 0, 0},
    {2, 0, 0x80},
    {2, 1, 0},
    {2, 1, 0x80},
...
void setBrightness(aux_t aux, displayBrightness_t *dp, int brightness)
{
    int ptr;
...
void setColorProfile(aux_t aux, displayBrightness_t *dp, int colorProfileNr)
{
    unsigned char *colorProfile;
