...
if [ -z "$MODULE_SOURCE" ] || [ ! -f "$MODULE_SOURCE" ] || \
   ...
   [ ! -f "$VCM_SOURCE" ]; then
	echo "Usage: sudo $0 /path/to/imx258.ko [/path/to/ipu-bridge.ko] [/path/to/intel_skl_int3472_discrete.ko] [/path/to/dw9806b.ko]" >&2
	exit 1
fi
...
echo "Rear-camera modules installed: $MODULE_DEST, $VCM_DEST, $BRIDGE_DEST and $INT3472_DEST"
